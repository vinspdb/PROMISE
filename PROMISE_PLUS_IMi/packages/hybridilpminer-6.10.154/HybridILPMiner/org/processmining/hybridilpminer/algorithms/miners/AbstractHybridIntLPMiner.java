...
public abstract class AbstractHybridIntLPMiner<T extends LPDecorator, S extends HybridVariableMapping<Integer>, U>
	...
	protected void findSourcePlaces() {
		...
		while (!(potentialSources.isEmpty())) {
			LPInstance instance;
			try {
				instance = new HybridSourcePlaceLPInstanceImpl<HybridVariableMapping<Integer>, Integer>(varMap,
						decorator.engine().clone(), potentialSources);
				instance.run();
	...
	public Pair<Petrinet, Marking> synthesizeNet() {
		...
		for (U event : logAbstraction.domain()) {
			int index = logAbstraction.encode(event);
			if (!decorator.getInvalidActivityIndices().contains(index)) {
